...
const SCREENS = {
  ...
  vorarbeiter:"./screens/vorarbeiter.html"
};
...
function bindBrandEasterEgg(){
  ...
  el.addEventListener("pointerdown", () => {
    t = setTimeout(() => { go("#/vorarbeiter"); }, 800);
  });
...
function fmtCallStatus(s){
  if(s === "offen") return "Offen";
  if(s === "ack") return "Gesehen";
...
async function route(){
  ...
  if(page === "vorarbeiter"){
    await renderScreen(SCREENS.vorarbeiter);
    bindVorarbeiter();
    return;
  }
...
function bindAuftraege(){
  ...
  const btn = document.getElementById("btnCallForemanDash");
  if(btn){
    btn.addEventListener("click", () => {
      createCall("dashboard");
      setStatus("Vorarbeiter gerufen");
      go("#/vorarbeiter");
    });
  }
...
function bindDetail(jobId){
  ...
  document.getElementById("btnStop").addEventListener("click", () => {
    ...
    const sessionStartTs = js.active.startTs;
    const sessionEndTs = now();
    const addMs = Date.now() - js.active.startTs;
    js.spentMs = (js.spentMs||0) + addMs;
    js.doneQty = (js.doneQty||0) + qty;
    js.history.push({ type:"stop", ts: sessionEndTs, startTs: sessionStartTs, endTs: sessionEndTs, qty, addMs });
    js.active = null;
  ...
  document.getElementById("btnFinish").addEventListener("click", () => {
    ...
    let sessionEndTs = now();
    if(js.active){
      sessionStartTs = js.active.startTs;
    ...
    js.status = "done";
    js.history.push({ type:"finish", ts: sessionEndTs, startTs: sessionStartTs, endTs: sessionEndTs, qty: finalQty, addMs });
    inpQty.value = "";
    persist();
    compute();
    setStatus("Fertig");
...
function bindVorarbeiter(){
  const pill = document.getElementById("pillCalls");
  const list = document.getElementById("callsList");

  state.calls = Array.isArray(state.calls) ? state.calls : [];

  function render(){
    const open = state.calls.filter(c => c.status !== "erledigt").length;
    if(pill) pill.textContent = `${open} offen`;

    list.innerHTML = "";
    if(state.calls.length === 0){
      const div = document.createElement("div");
      div.className = "item";
      div.innerHTML = `<div class="item__title">Keine Rufe</div><div class="item__sub">Wenn jemand ruft, erscheint es hier.</div>`;
      list.appendChild(div);
      return;
    }

    state.calls.slice().reverse().forEach(c => {
      const when = new Date(c.ts);
      const hh = String(when.getHours()).padStart(2,"0");
      const mm = String(when.getMinutes()).padStart(2,"0");
      const title = c.jobId === "dashboard" ? "Dashboard" : `Auftrag ${c.jobId}`;

      const item = document.createElement("div");
      item.className = "item";
      item.innerHTML = `
        <div class="item__title">${c.userName} • ${title}</div>
        <div class="item__sub">${hh}:${mm} • Status ${fmtCallStatus(c.status)}</div>
        <div class="row">
          <button class="btn btn--ghost" data-s="ack">Gesehen</button>
          <button class="btn btn--ghost" data-s="unterwegs">Unterwegs</button>
          <button class="btn btn--good" data-s="erledigt">Erledigt</button>
          <button class="btn btn--primary" data-open>Öffnen</button>
        </div>
      `;

      item.querySelectorAll("[data-s]").forEach(b => {
        b.addEventListener("click", () => {
          c.status = b.getAttribute("data-s");
          persist();
          render();
      ...
      item.querySelector("[data-open]")?.addEventListener("click", () => {
        if(c.jobId && c.jobId !== "dashboard") go(`#/auftrag/${c.jobId}`);
        else go("#/auftraege");
      });
